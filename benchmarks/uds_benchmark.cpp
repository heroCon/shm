...
int main(int argc, char** argv) {
  ...
  int fd[2];
  if (bytes == 0 || socketpair(AF_UNIX, SOCK_DGRAM, 0, fd)) return 2;
  std::string data(bytes, 'x');
  auto start = std::chrono::steady_clock::now();
  std::thread receiver([&] {
    std::string out(bytes, '\0');
    for (size_t i = 0; i < count; ++i)
      if (recv(fd[1], &out[0], bytes, 0) < 0) std::abort();
  });
  for (size_t i = 0; i < count; ++i)
    if (send(fd[0], data.data(), bytes, 0) < 0) return 3;
  receiver.join();
  auto ns =
